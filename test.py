...
def RNN(X, weights, biases):
    ...
    ##########################################
    # basic LSTM Cell.
    fw_cell = tf.contrib.rnn.BasicLSTMCell(n_hidden_units)
    bw_cell = tf.contrib.rnn.BasicLSTMCell(n_hidden_units)
    outputs, _, _ = tf.contrib.rnn.static_bidirectional_rnn(fw_cell, bw_cell, X_in,
                                                            dtype=tf.float32)
    X_out = tf.transpose(outputs[-4:], [1, 0, 2])
    X_out = tf.reshape(X_out, [-1, 2*n_hidden_units])
